...
class AutoSegLoss(nn.Module):
        ...
        for n in range(0, len(cp), 4):
            if n == 0:
                ...
                if self.inplace:
                    ...
                    y = t.mul(2 * cp[n + 1] - 2 * cp[n - 1]).add_(
                        t.square().mul_(cp[n - 1] - 2 * cp[n + 1] + cp[n + 3])).add_(cp[n - 1])
                else:
                    t = self._quadratic_solver(cp[n - 2], cp[n], cp[n + 2], x_part)
    ...
    def _forward_metric(self, pred, target_onehot, mask, theta):

        if self.target_metric == 'mIoU':
            pred_metric = forward_mIoU(pred, target_onehot, mask, self.parameterize, theta, self.eps)

        elif self.target_metric == 'FWIoU':
            pred_metric = forward_FWIoU(pred, target_onehot, mask, self.parameterize, theta, self.eps)

        elif self.target_metric == 'BIoU':
            pred_metric = forward_BIoU(pred, target_onehot, mask, self.parameterize, theta, self.eps, self.drop_bg,
                                       self.tol)

        elif self.target_metric == 'mAcc':
            pred_metric = forward_mAcc(pred, target_onehot, mask, self.parameterize, theta, self.eps)

        elif self.target_metric == 'gAcc':
            pred_metric = forward_gAcc(pred, target_onehot, mask, self.parameterize, theta, self.eps)

        elif self.target_metric == 'BF1':
            pred_metric = forward_BF1(pred, target_onehot, mask, self.parameterize, theta, self.eps, self.drop_bg,
                                      self.tol)
    ...
    def forward(self,
        ...
        pred_metric = self._forward_metric(pred, target_onehot, mask, theta_device)
...
def forward_BF1(input, target, mask, parameterize, theta, eps, drop_bg, tol):
    ...
    union = parameterize(input, theta_3) + parameterize(input_erode, theta_4) - parameterize(input,
                                                                                             theta_3) * parameterize(
        input_erode, theta_4)
